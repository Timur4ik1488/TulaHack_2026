...
async def update_team(
    ...
    if not data:
        return team
    if "name" in data:
        exists = await db.execute(select(Team).where(Team.name == data["name"], Team.id != team_id))
...
@router.delete("/{team_id}", status_code=status.HTTP_204_NO_CONTENT, dependencies=[Depends(is_user_admin)])
