...
export function teamPhotoSrc(url: string | null | undefined): string {
  const u = url?.trim()
  if (u) return u
  return DEFAULT_TEAM_AVATAR
}

export function onTeamPhotoError(ev: Event) {
...
export function userAvatarSrc(url: string | null | undefined): string {
  const u = url?.trim()
  if (u) return u
  return DEFAULT_TEAM_AVATAR
}
